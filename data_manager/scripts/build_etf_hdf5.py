...
from config import DATA_DIR, BACKTEST_ETF_BASE

# 定义 ETF 数据路径
ETF_RAW_DIR = os.path.join(DATA_DIR, "etf", "raw")
ETF_RAW_H5_DIR = os.path.join(DATA_DIR, "etf", "raw_h5")

def build():
    print(f"开始分批写入 {ETF_RAW_DIR} 下的 csv 文件到 HDF5...")
    csv_files = glob.glob(os.path.join(ETF_RAW_DIR, "*.csv"))

    if not csv_files:
        print("未找到任何 CSV 文件！请确保 ETF_RAW_DIR 路径正确且包含数据。")
        return

    # 1. 提取最早和最晚的交易日期
    first_valid_file = csv_files[0]
    print(f"正在读取文件 {first_valid_file} 以确定数据日期范围...")
    ...
    OUTPUT_HDF = os.path.join(ETF_RAW_H5_DIR, data_time, "market_data.h5")
    ...
    for i in range(0, len(csv_files), batch_size):
        ...
        for f in batch:
            ...
            try:
                ...
                valid_cols = [c for c in header if c in BACKTEST_ETF_BASE]
                ...
                df = df.copy()
                # 兼容聚宽等后补后缀的要求，还原为带后缀的 ts_code (可选)
                # 若需要带后缀，可自行添加判断，此处以文件名直接作为 ts_code
                df['ts_code'] = symbol 
                df_list.append(df)
            except Exception as e:
                print(f"读取 {symbol} 失败: {e}")
            
        print(f"合并第 {i} 到 {min(i+len(batch), total_files)} 个文件...")
        if not df_list: 
            continue
    ...
    print("-" * 50)
    print(f"ETF 数据 HDF5 文件构建成功！")
    print(f"实际加载数: {total_files}")
    print(f"输出路径: {OUTPUT_HDF}")
    print("-" * 50)
